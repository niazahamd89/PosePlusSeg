...
def model(inputs,input_height,input_width):
    ...
    kp_maps = tf.contrib.layers.conv2d(net,num_outputs = config.NUM_KP,
                                             kernel_size=(1,1),activation_fn=tf.nn.sigmoid,stride=1,scope='kp_maps',reuse=tf.compat.v1.AUTO_REUSE)
    short_offsets = tf.contrib.layers.conv2d(net,num_outputs = 2*config.NUM_KP,
                                             kernel_size=(1,1),activation_fn=None,stride=1,scope='short_offsets',reuse=tf.compat.v1.AUTO_REUSE)
    mid_offsets = tf.contrib.layers.conv2d(net,num_outputs = 4*config.NUM_EDGES,
                                             kernel_size=(1,1),activation_fn=None,stride=1,scope='mid_offsets',reuse=tf.compat.v1.AUTO_REUSE)
    long_offsets = tf.contrib.layers.conv2d(net,num_outputs = 2*config.NUM_KP,
                                             kernel_size=(1,1),activation_fn=None,stride=1,scope='long_offsets',reuse=tf.compat.v1.AUTO_REUSE)
    seg_mask = tf.contrib.layers.conv2d(net,num_outputs = 1,
                                             kernel_size=(1,1),activation_fn=tf.nn.sigmoid,stride=1,scope='seg_mask',reuse=tf.compat.v1.AUTO_REUSE)
 
    kp_maps = tf.compat.v1.image.resize_bilinear(kp_maps, (height,width), align_corners=True)
    short_offsets = tf.compat.v1.image.resize_bilinear(short_offsets, (height,width), align_corners=True)
    mid_offsets = tf.compat.v1.image.resize_bilinear(mid_offsets, (height,width), align_corners=True)
    long_offsets = tf.compat.v1.image.resize_bilinear(long_offsets, (height,width), align_corners=True)
    seg_mask = tf.compat.v1.image.resize_bilinear(seg_mask, (height,width), align_corners=True)
