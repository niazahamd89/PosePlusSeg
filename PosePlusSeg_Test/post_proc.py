...
def compute_heatmaps(kp_maps, short_offsets):
    heatmaps = []
    #kp_maps = maximum_filter(kp_maps, size=2)
...
def get_keypoints(heatmaps):
    ...
    for i in range(config.NUM_KP):
        ...
        peaks =maximum_filter(heatmaps[:,:,i], size=20, mode='mirror') == heatmaps[:,:,i]
...
def group_skeletons(keypoints, mid_offsets,img_id):
    keypoints.sort(key=(lambda kp: kp['conf']), reverse=True)
    skeletons = []
    dir_edges = config.EDGES + [edge[::-1] for edge in config.EDGES]
    ...
    while len(keypoints) > 0:
        ...
        for edge in path:
            ...
            # print(str(edge) +" : "+ str(matches[0][1]['xy'])+ "\n")
            keypoints.pop(matches[0][0])
            this_skel[edge[1],:2] = to_kp
            this_skel[edge[1], 2] = to_kp_conf
            # kp_array_pop = kp_array.pop()
            # print(kp_array_pop)
            # print(len(kp_array_pop))
            
            # if len(kp_array_pop) > 1:
            #     k = 0
            #     for kp_array_pop_iter in kp_array_pop:
            #         # if kp_array_pop[k][1]['conf'] > kp_array_pop[k+1][1]['conf']:
            #         #     del kp_array_pop[int(k+1)]
            #         # else:
            #         #     del kp_array_pop[int(k)]
            #         # print(type(kp_array_pop_iter[k][1]['conf']))
            #         print(kp_array_pop_iter)
            #         k = k + 1
            #         if (k+1) > len(kp_array_pop):
            #             break
                    

                        
            # print(str(matches[0][1]['id'])+" : "+str(matches[0][1]['conf']))
            # print(" ")
            # result = open('keypoints.txt','a')
            # result.write(str(kp_array_pop))
            # result.write("\n")
            # result.write(str(len(kp_array_pop)))
            # result.write("\n")
            # result.write(str(matches[0][1]['id'])+" : "+str(matches[0][1]['conf']))
            # result.write("\n")

            # result.close()
            
            # kp_array.append(kp_array_pop)
            kp_array.append(matches)
            # print(str(kp) + " : " + str(matches))
        if len(kp_array) > 3: 
            result = open('keypoints_1pic.txt','a')
            result.write(str(img_id)+ "|" + str(kp_array))
            # print(kp_array)
            result.write("\n")
            result.close()
...
def get_instance_masks(skeletons, seg_mask, long_offsets, threshold=True):
    ...
    # result = open('segmentation.txt','w')
    # result.write(str(seg_mask))
    # result.write("\n")
    # result.close()
